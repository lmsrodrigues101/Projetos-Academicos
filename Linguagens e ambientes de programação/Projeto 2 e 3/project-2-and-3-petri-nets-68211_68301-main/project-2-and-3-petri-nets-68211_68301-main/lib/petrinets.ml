...
let compare_arcs (a1:arc) (a2:arc) = 
  ...
  |(Outgoing_place_arc(p1,t1,w1), Outgoing_place_arc(p2,t2,w2)) -> 
    (compare_transitions t1 t2) + (compare_places p1 p2) + (w1-w2)
  | _ , _ -> 1
...
(*recebe um place,capacity,tokens,preti_net. Inicializa a place com a c e t e se
 a place ja existir na substitui*)
...
(*Adiciona uma transition à petri net, caso essa transition já exista
 é substituida*)
...
(** [verify_p name net] checks if the place with name [name] exists  
    in the petri net [net]. Returns [true] if it exists, [false] otherwise. *)
let verify_p name net = 
  List.exists(fun (h,_,_) -> h.name_p = name ) (net.places)

(** [verify_t name net] checks if the transition with name [name] exists  
    in the petri net [net]. Returns [true] if it exists, [false] otherwise. *)
let verify_t name net = 
  List.exists(fun h -> h.name_t = name ) (transitions net)
  
(*Adiciona um arc à petri net, retorna None caso o arc não esteja
 bem definido na petri net*)
let add_arc (a:arc) net =
 match a with 
  | Outgoing_place_arc(p,t,_) | Incoming_place_arc(t,p,_) -> 
    if verify_p p.name_p net && verify_t t.name_t net then
       Some {net with arcs = a :: arcs net}
    else None


(** [place_info p net] retrieves information about place [p] in the petri 
net [net].Returns [Some (capacity, tokens)] if the place exists,
otherwise [None]. *)   
...
(*Retorn o numero de tokens do place p na petri net (net) dada, caso o place 
não exista na petri net retorna None*)
...
(** [capacity_of_place net p] returns the maximum token capacity of place [p]  
    in the petri net [net]. If the place does not exist, returns [max_int]. *)
...
(** [preceding_places t net] returns the list of (places,weight) that
 have outgoing arcs towards transition [t] in the petri net [net]. *)
...
(** [succeding_places t net] returns the list of (places,weight) that receive 
tokens from transition [t] in the petri net [net]. *)   
...
(** [has_tokens (p, w) net] checks if place [p] in the petri net [net]  
    has at least [w] tokens available for transition firing. *)
let has_tokens (p,w) net =  
  match marking_of_place p net with
  | Some tokens -> tokens >= w
  | None -> false

(** [can_get_tokens (p, w) net] checks if place [p] in the petri net [net]  
    can receive [w] tokens without exceeding its capacity. *)
...
(*para todos os arcos de outgoing o place p desse arco tem que ter pelo menos
 (>=) w tokens, para todos os arcos incoming o place p tem de ter capacidade
...
(** [remove_tokens (p, w) net] removes [w] tokens from place [p]  
    in the petri net [net]. *)
...
(** [add_tokens (p, w) net] adds [w] tokens to place [p]  
    in the petri net [net], ensuring it does not exceed its capacity. *)
...
(*1ºVerificar se a transition existe na net, e verificar se está enabled
  2º atualizar os places: para outgoing subtrair o nº de tokens indicado
   pelo weigth do place correspondente
  para incoming adicionar o nº de tokens indicado para o place 
  3º garantir que ao atualizar as marcações não tem tokens a mais e nem
   tokens negativos
  4º retornar uma nova petri net com tudo atualizado*)  
let fire t net = 
  if not(verify_t t.name_t net) || not (is_enabled t net) then None
  else 
    let net_after_removes =
       List.fold_left (fun acc place_w -> remove_tokens place_w acc) net (preceding_places t net) in
    let net_after_addition = 
      List.fold_left (fun acc place_w -> add_tokens place_w acc) net_after_removes (succeding_places t net) in
    Some net_after_addition

  (* fazer o fire para todos as transitions*)
